...
# Indlæser AMD regnskaber
folder <- "/Users/marius/Documents/DataAnalyseProjekter/NLP & Deep-Learning/NLP & Deep-Leaning R/AMD" 
...
pdf_list[[1]] <- pdf_list[[1]][2:3]
pdf_list[[2]] <- pdf_list[[2]][3:5]
pdf_list[[3]] <- pdf_list[[3]][3:5]
pdf_list[[4]] <- pdf_list[[4]][2:4]
pdf_list[[5]] <- pdf_list[[5]][2:4]
pdf_list[[6]] <- pdf_list[[6]][2:5]
pdf_list[[7]] <- pdf_list[[7]][2:5]
pdf_list[[8]] <- pdf_list[[8]][3:7]
...
stock_data <- stock_prices %>%
  ...
  group_by(year) %>%
  mutate(days_from_mar1 = abs(as.numeric(date - as.Date(paste(year, "-03-01", sep = ""))))) %>%
  slice(which.min(days_from_mar1)) %>%
  ungroup()
...
result_df_pct$Årstal <- as.Date(paste(result_df_pct$Årstal, "-03-01", sep = ""))

# Plotter resultat
amd_plot <- ggplot(data = result_df_pct, aes(x = Årstal)) +
  ...
  scale_color_manual(values = c("Sentiment-score" = "#ED1C24", "Afkast" = "#5e5e5e")) +
  labs(title = "Ingen betydelig sammenhæng mellem sentiment score og AMD's aktiekurs",
       subtitle = "Afkast & Sentiment-score sammenlignet med gennemsnit \nSentiment-score beregnet på AMD's stakeholder statement i årsrapport",
       x = "",
       y = "",
       caption = "Data: Yahoo Finance & AMD's årsrapport") +
...
ggsave(filename = "amd_plot.jpeg", plot = amd_plot, width = 8, height = 5, dpi = 300)
